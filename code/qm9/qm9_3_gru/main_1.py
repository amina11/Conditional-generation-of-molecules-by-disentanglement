...
import time
for epoch in range(460, cmd_args.num_epochs):
    start = time.time()
    random.shuffle(sample_idxes)
    ## update the vae:
    for epoch_vae in range(5):
        ae, vae_loss = epoch_train('train',epoch_vae, ae, regressor, sample_idxes, train_binary_x, train_masks, train_y,cmd_args, optimizer_encoder, optimizer_decoder)
    ...
    for epoch_regressor in range(5):
        regressor, regressor_loss, reg_1= train_regressor('train', epoch_regressor, optimizer_regressor, ae, regressor,sample_idxes,  train_binary_x, train_y)
