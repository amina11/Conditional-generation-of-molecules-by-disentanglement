...
def epoch_train(phase, epoch, ae, regressor, sample_idxes, data_binary, data_masks, data_property, cmd_args, optimizer_encoder=None, optimizer_decoder = None):
    total_loss = []
...
from train_util import get_batch_input_vae
#q(z|x,y)
def epoch_train_icml(phase, epoch, ae, regressor, sample_idxes, data_binary, data_masks, data_property, cmd_args, optimizer_vae=None, optimizer_regularizer = None):
    total_loss = []
...
def train_regressor(phase, epoch, optimizer, model, regressor, sample_indexs, data_binary, data_property):
    ...
    for pos in pbar:
        ...
        x_inputs, y_inputs,v_x, t_y = get_batch_input_regressor(selected_idx, data_binary, data_property) 
        z_mean, z_log_var = model.encoder(x_inputs)
        #z = model.reparameterize(z_mean,  z_log_var)
        z = z_mean
        ## p(x|z, y')
