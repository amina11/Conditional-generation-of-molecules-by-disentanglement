...
import time
for epoch in range(1412, cmd_args.num_epochs):
    start = time.time()
    random.shuffle(sample_idxes)
    ## update the vae:
    
    ae, vae_loss = epoch_train('train',epoch, ae, regressor, sample_idxes, train_inputs, train_binary, train_masks, train_y,cmd_args, optimizer_encoder, optimizer_decoder)
