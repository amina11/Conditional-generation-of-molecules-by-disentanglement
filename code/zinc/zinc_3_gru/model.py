...
def Deterministic_policy_regularizer(model, regressor, x_inputs, y_inputs, true_binary, rule_masks, t_y):   
        ...
        original_reward = ((pred_y.float().view(-1) - t_y)**2).sum() #sum of square error 
        ...
        permuted_reward = ((pred_y_permuted.float().view(-1) - permuted_ty_)**2).sum() #sum of square error 
        '''
...
class StateDecoder(nn.Module):
    def __init__(self, max_len, latent_dim):
        ...
        self.z_to_latent = nn.Linear(self.latent_dim + cmd_args.output_dim, self.latent_dim)
        if cmd_args.rnn_type == 'gru':
            self.gru_1 = nn.GRU(self.latent_dim + cmd_args.output_dim , 501, 1)
            self.gru_2 = nn.GRU(501 + 1 , 501, 1)
            self.gru_3 = nn.GRU(501 + 1  , 501, 1)            
    ...
    def forward(self, z, y):
        ...
        ## relu(h),y]
        h = torch.cat((F.relu(h), y.view(y.shape[0],1)), 1)
        rep_h = h.expand(self.max_len, z.size()[0], z.size()[1] + cmd_args.output_dim) # repeat along time steps
        out_1, _ = self.gru_1(rep_h)# run first-layer gru
        ## apply second gru
        y_1 = y.reshape(1,y.shape[0])
        y_1 = y_1.repeat(self.max_len, 1)
        y_1 = y_1.reshape(self.max_len, y.shape[0], 1)
        h_1 = torch.cat((out_1,y_1), 2)
        out_2,h_2 = self.gru_2(h_1)
        ## apply third gru
        h_2 = torch.cat((out_2, y_1), 2)
        out_3, _ = self.gru_3(h_2)
        
        
        logits = self.decoded_logits(out_3)
        return logits   
...
def epoch_train(phase, epoch, ae, regressor, sample_idxes,train_inputs, train_binary, train_masks, train_y, cmd_args, optimizer_encoder=None, optimizer_decoder = None):
    total_loss = []
    #pbar = tqdm(range(0, (len(sample_idxes) + (cmd_args.batch_size - 1) * (optimizer_encoder is None)) // cmd_args.batch_size), unit='batch')
    pbar = range(0, (len(sample_idxes) + (cmd_args.batch_size - 1) * (optimizer_encoder is None)) // cmd_args.batch_size)
    ...
    for pos in pbar:
        ...
        x_inputs, y_inputs,v_tb, v_ms, t_y = get_batch_input_vae(selected_idx,  train_inputs, train_binary, train_masks, train_y)
        ...
        loss_encoder = loss_list[0] + loss_list[1] + cmd_args.RL_param * original_reward
        loss_decoder = loss_list[0] + loss_list[1] + cmd_args.RL_param * (original_reward + permuted_reward)

        minibatch_vae_loss = (loss_list[0] + loss_list[1]).data.cpu().numpy()
        minibatch_regularizer = cmd_args.RL_param * (original_reward.data.cpu().numpy() + permuted_reward.data.cpu().numpy())
        #pbar.set_description('At epoch: %d  %s vae loss: %0.5f regularizer loss: %0.5f original reward: %0.5f  permuted reward: %0.5f perp: %0.5f kl: %0.5f' % (epoch, phase, minibatch_vae_loss, minibatch_regularizer, original_reward, permuted_reward, perp, kl))            
        ...
        total_loss.append(np.array([minibatch_vae_loss, minibatch_regularizer,  original_reward, permuted_reward, perp, kl]) * len(selected_idx))       
...
from train_util import get_batch_input_vae
#q(z|x,y)
def epoch_train_icml(phase, epoch, ae, regressor, sample_idxes,  x_inputs, data_binary, data_masks, data_property, cmd_args, optimizer_vae=None, optimizer_regularizer = None):
    total_loss = []
    ...
    for pos in pbar:
        selected_idx = sample_idxes[pos * cmd_args.batch_size : (pos + 1) * cmd_args.batch_size]
        x_inputs, y_inputs,v_tb, v_ms, t_y = get_batch_input_vae(selected_idx,  x_inputs, data_binary, data_masks, data_property)  # no grad for evaluate mode.              
        loss_list = ae.forward(x_inputs, y_inputs,v_tb,v_ms, t_y)
        perp = loss_list[0].data.cpu().numpy()[0] # reconstruction loss
...
def train_regressor(phase, epoch, optimizer, model, regressor, sample_indexs,  x_train, y_train):
    total_loss = []
    #pbar = tqdm(range(0, (len(sample_indexs) + (cmd_args.batch_size - 1) * (optimizer is None)) // cmd_args.batch_size), unit='batch')
    pbar = range(0, (len(sample_indexs) + (cmd_args.batch_size - 1) * (optimizer is None)) // cmd_args.batch_size)
    ...
    for pos in pbar:
        ...
        x_inputs, y_inputs,v_x, t_y = get_batch_input_regressor(selected_idx,  x_train, y_train) 
        z_mean, z_log_var = model.encoder(x_inputs)
        #z = model.reparameterize(z_mean,  z_log_var)
        z = z_mean
        ## p(x|z, y')
       ...
       '''
        ...
        true_y = t_y
        negative_reward = ((pred_y.float().view(-1) - true_y)**2).sum() #sum of square error 
        minibatch_loss = negative_reward.data.cpu().numpy()
        #pbar.set_description('At epoch: %d  regressor loss: %0.5f' % (epoch, minibatch_loss))
        #pbar.set_description('At epoch: %d  regressor loss: %0.5f  reg1: %0.5f' % (epoch, minibatch_loss, negative_reward))
