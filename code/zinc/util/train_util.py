...
class Prepare_data():
    def __init__(self, cmd_args):
        self.data_dir = cmd_args.training_data_dir + '/zinc_clean_smi_train.h5'
        self.label_dir = cmd_args.training_data_dir + '/zinc_normalized_train_y.npy'
    ...
    def pre_process(self, data_binary, data_masks, data_property):
        true_binary = np.transpose(data_binary, [1, 0, 2])
        rule_masks = np.transpose(data_masks, [1, 0, 2])
...
def get_batch_input_regressor(selected_idx, x_train, y_train, volatile=False):
    ...
    x_inputs = (x_train[selected_idx, :, :]).astype(np.float32)
...
def get_batch_input_vae(selected_idx, x_train, data_binary, data_masks, data_property):
    x_inputs =  (x_train[selected_idx, :, :]).astype(np.float32)
    true_binary = (data_binary[:,selected_idx,:]).astype(np.float32)
    rule_masks = (data_masks[:,selected_idx,:]).astype(np.float32)
